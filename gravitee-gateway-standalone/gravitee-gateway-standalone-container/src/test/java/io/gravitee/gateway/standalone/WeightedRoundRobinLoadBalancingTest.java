...
public class WeightedRoundRobinLoadBalancingTest extends AbstractGatewayTest {
    ...
    public void call_weighted_lb_multiple_endpoints() throws Exception {
        stubFor(get(urlEqualTo("/api1")).willReturn(ok()));
        stubFor(get(urlEqualTo("/api2")).willReturn(ok()));
        ...
        verify(3, getRequestedFor(urlPathEqualTo("/api1")));
        verify(7, getRequestedFor(urlPathEqualTo("/api2")));
    }
}
